...
add-edge)
  ...
  'edges:' block to paste into the management site's values.yaml."
    ...
    NEW_DOC="$(cat <<EOF2
---
# This edge's S3 identity, generated by 'site-secrets.sh add-edge'. The SAME
# key pair is in sites/${EDGE}/secrets.enc.yaml as s3-edge-credentials — do
# not let the two drift; cert-sync does not deliver this one, only the Loki
# client certificate.
apiVersion: v1
kind: Secret
metadata:
...
EOF2
    ...
    if grep -q '^sops:' "$MGMT_SECRETS"; then
