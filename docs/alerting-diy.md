...
| How do I send to a different inbox? | Set `observability.stack.alerting.*` in your site file; re-run `./install.sh <site>`. For a different *route or receiver*, edit `charts/edge/files/alertmanager-config.yaml` — **not** `kube-prometheus-stack.alertmanager.config`, which renders a Secret nothing mounts (§3) |
...
The admin password is **not** generated by the chart, and there is no
`<release>-grafana` Secret to read it from. `charts/edge/values.yaml` wires
`kube-prometheus-stack.grafana.admin.existingSecret: grafana-admin-credentials`
unconditionally (`userKey: admin-user`, `passwordKey: admin-password`), so the
Grafana Deployment's `GF_SECURITY_ADMIN_USER` / `GF_SECURITY_ADMIN_PASSWORD` are
`secretKeyRef`s onto that Secret and the subchart never renders one of its own.
Asking for `<release>-grafana` returns `NotFound`, which reads like a broken
install and is not. The password is whatever your site put in
`sites/<site>/secrets.enc.yaml`:
...
If that Secret is absent Grafana does not quietly fall back to a generated
password — the env refs are not optional, so the pod sits in
`CreateContainerConfigError` and never serves the NodePort at all. Create it
with `scripts/site-secrets.sh` before blaming Grafana.
...
instead of a hand-applied object, `charts/edge` does template rules, so it can
ship with the chart:

- **Prometheus.** Drop a bare `groups:` fragment — no `apiVersion`, no `kind`,
  no `metadata`; `templates/observability.yaml` supplies those, including the
  namespace — into `charts/edge/files/prometheus-rules/<severity>.yaml`. The
  template globs that directory and renders one `PrometheusRule` per file,
  named `ais-edge-<severity>`. The three that ship (`critical`, `warning`,
  `info`) carry the header comment describing the fragment shape; copy it.
- **Loki.** Add the group to `charts/edge/files/loki-ruler-rules.yaml`, which
  renders as the ConfigMap `<release>-loki-rules` labelled `loki_rule`. Leave
  the `__DP_MIN_FREE_DISK_PCT__` / `__DP_QUARANTINE_ALERT_AFTER_S__` sentinels
  in that file alone — the template substitutes them from `dataPolicy.originals`
  at render time, and an unreplaced sentinel is a rule-load syntax error that
  makes Loki drop the whole group.

The trade-off is timing, not capability: a chart-file edit reaches the cluster
only on the next `helm upgrade` — that is, the next `./install.sh <site>` (Step
4) — whereas `kubectl apply` takes effect within seconds. Use the chart for a
rule every site should have, and `kubectl apply` for a one-off you do not want
in the release.
...
> CAUTION, and this one is silent if you break it: the two halves have to
> agree, and in `charts/edge` they already do. The sidecar writes into
> `/rules/fake` and `loki.loki.rulerConfig.storage.local.directory` is `/rules`
> — the tenant sub-directory is what closes the gap, because Loki's local rule
> store reads every sub-directory of that path as a tenant ID and with
> `auth_enabled: false` the only tenant is literally `fake`. A rules file left
> at the top level of `/rules` is not an error: Loki starts happily, the rules
> API returns an empty set, and no LogQL alert ever fires.
>
> This is what is shipped, so **do not copy it into your site file** — you
> would only pin values that already hold, and pinning one half without the
> other is exactly how the pair comes apart:
>
> ```yaml
> # charts/edge/values.yaml — already set, shown so you recognise the pair
> loki:
>   sidecar:
>     rules:
...
> ```
>
> The obvious-looking `/etc/loki/rules` was tried and measured: that path is
> read-only in the image, Loki mkdir's the ruler directory at startup, and the
> result was `mkdir /etc/loki/rules: read-only file system` /
> `error initialising module: ruler-storage` and a CrashLoop — no log storage
> and no pipeline alerts. `helm template` cannot catch that class of mistake,
> because the path is wrong only relative to where the *subchart* mounts things.

`charts/edge` already ships a ConfigMap of this shape —
`<release>-loki-rules`, rendered from `charts/edge/files/loki-ruler-rules.yaml`
with 11 alerts in four groups. Yours is an **additional** ConfigMap carrying the
same label, not an edit to that one: the sidecar selects on the label, so any
number of them load side by side.
...
Alertmanager belongs to the kube-prometheus-stack subchart, but its routing does
**not** come from the subchart's values. `charts/edge` keeps its own config file
— `charts/edge/files/alertmanager-config.yaml` — which
`charts/edge/templates/observability.yaml` renders into the Secret
`alertmanager-aisedge-config`, and `charts/edge/values.yaml` pins
`kube-prometheus-stack.alertmanager.alertmanagerSpec.configSecret` at that name.
`configSecret` is mounted by the operator as the whole `alertmanager.yaml`, and
it wins over anything set under `kube-prometheus-stack.alertmanager.config`:
that key still renders the subchart's own `alertmanager-ais-kps-alertmanager`
Secret, but with `configSecret` set nothing ever mounts it. Setting routes there
is not "overridden later" — it is silently inert.

So the two knobs are different in kind. **Mail facts** come from the site file;
**routes and receivers** come from the chart file.
...
`username`, `password`), encrypted, and reaches the pod as a mounted file at
`/etc/alertmanager/secrets/alertmanager-smtp/password`. The template writes that
path into the config as `smtp_auth_password_file`, so the credential never lands
in a rendered manifest, in git, or in `helm get values`.

> CAUTION: verify against the Secret that is actually mounted. That is
> `alertmanager-aisedge-config`, **not** `alertmanager-ais-kps-alertmanager` —
> the latter is the subchart's default, whose route ends at `receiver: "null"`,
> and reading it will "confirm" a null route that never runs. A null receiver
> accepts every alert and delivers nothing, with no error anywhere:
> Alertmanager shows the alert firing and the inbox stays empty. That is the
> failure mode you get if `configSecret` is ever dropped, which is why
> `templates/observability.yaml` `fail`s the render without it.
>
> ```bash
...
To change routes and receivers, edit `charts/edge/files/alertmanager-config.yaml`
and re-run `./install.sh <site>`. Two rules about that file, both load-bearing:

- **It is never rendered with `tpl`.** It is full of Alertmanager's own `{{ }}`
  notification templates (`{{ .CommonLabels.session }}` and friends). Running
  `tpl` over it would evaluate those against the *chart* context, where they do
  not exist, and they would render as empty strings — every alert arriving with
  a blank body and nothing erroring. The installer's values are spliced in as
  `__SENTINEL__` tokens by a plain `replace` instead. If a new value has to come
  from the site file, add a new sentinel; do not reach for `tpl`.
- **`configSecret` and `alertmanagerSpec.secrets: [alertmanager-smtp]` are
  already set** in `charts/edge/values.yaml` and are not yours to add.
  `templates/observability.yaml` `fail`s the render if `configSecret` names
  anything else, and again if the SMTP Secret is missing from the mount list —
  because without the mount `smtp_auth_password_file` points at nothing and
  every notification fails authentication.

The shipped tree, for orientation before you edit it:

```yaml
# charts/edge/files/alertmanager-config.yaml (excerpt)
route:
  ...
  routes:
    ...
    #
    # kube-prometheus-stack's own meta-alerts go FIRST, so they can never
    # fall through to the tree below. Both fire permanently by design and
    # say nothing about this site; they mailed the operator forever until
    # this route was added.
    - matchers: ['alertname =~ "Watchdog|InfoInhibitor"']
      receiver: "null-meta"
    - matchers: ['alertname = "DICOMRejectedUnmappedAET"']
...
receivers:
  ...
  - name: email-primary
    email_configs:
      - to: "__ALERT_EMAIL_TO__"     # ← observability.stack.alerting.emailTo
        send_resolved: true
...
a *placement* problem, not a bad rule — check that the file landed under
`/rules/fake` and not at the top of `/rules` (the tenant sub-directory from Step
3). The other shape worth knowing: if the rules API is missing your group **and**
the 11 that ship, the file did not parse. Loki rejects the whole file on the
first malformed group, so one rule indented out of its `rules:` list takes every
other rule in that file with it — `scripts/ci/promtool.sh` catches exactly this
before install.
...
3. **Confirm delivery.** Check the inbox / Slack channel. If nothing
   ...
   - Is the route reaching a real receiver? Read the mounted config, not the
     subchart's — `alertmanager-aisedge-config` (§3). `null-meta` is a real
     receiver that delivers nothing on purpose; anything routed there is
...
| File | What's in it |
|---|---|
| `sites/<site>/values.yaml` | `observability.stack.enabled` (the on/off switch for the whole stack — it gates both subcharts via `condition:` in `charts/edge/Chart.yaml`) and `observability.stack.alerting.*` (inbox, sender, SMTP host/port/user, `requireTLS`, `existingSecret`). Those are the only `observability.stack` keys any template reads — plus any `kube-prometheus-stack:` / `loki:` subchart overrides you add. Grafana's port is `kube-prometheus-stack.grafana.service.nodePort`; log retention is `loki.loki.limits_config.retention_period`; metric retention is `kube-prometheus-stack.prometheus.prometheusSpec.retention`. The friendly parent keys `observability.stack.grafana.nodePort` and `observability.stack.retentionDays` were **deleted** (see the comment block in `charts/edge/values.yaml`) because Helm cannot push a parent value into a subchart — they were read, trusted and silently ignored. `install.sh` still reads the first one to build the Grafana URL it prints, so setting it without also setting the subchart key just prints a link to a closed port |
| `charts/edge/files/prometheus-rules/{critical,warning,info}.yaml` | The chart's own Prometheus alerts, as bare `groups:` fragments. Rendered one `PrometheusRule` per file, named `ais-edge-<severity>` |
| `charts/edge/files/loki-ruler-rules.yaml` | The chart's own 11 LogQL alerts, in four groups. Rendered as ConfigMap `<release>-loki-rules` labelled `loki_rule`, with the `__DP_*__` thresholds substituted from `dataPolicy.originals` |
| `charts/edge/files/alertmanager-config.yaml` | Routes, receivers and inhibit rules. Rendered into Secret `alertmanager-aisedge-config` with `__SENTINEL__` tokens replaced from `observability.stack.alerting.*` — this is the config Alertmanager actually runs |
